...
module.exports = (function () {
    ...
    Odoo.prototype.rpc = function (path, cb, params, options) {
        ...
        // return setTimeout(json_client.request('call', params, _.uniqueId('r'), cb),1000)
    };
    ...
    Odoo.prototype._buildWhere = function (model, where) {
        ...
        Object.keys(where).forEach(function (k) {
            ...
            if (k === 'and'|| k === 'or' || k === 'nor') {
                ...
                var condition = k.replace(/and/, '&').replace(/or/, '|').replace(/nor/, '');
                query.unshift(condition);

                //to handle if the query has more than two conditions, add the | or & in the head of array
                 if((k==='or'|| k==='and'|| k === 'nor') && queryLen>2){
                     for(var index = 2; index < queryLen; index++){
                         query.unshift(condition);
                     }
                 }
                 //to handle the query with wrong params
                 if( (k==='or' || k ==='and' || k === 'nor') && queryLen<2){
                     throw new Error(g.f('Condition should not have less than two parameters'));
                 }
            ...
            if(!prop){
                throw new Error(g.f('Could not find the property name in the definition'));
            }
            // console.dir(prop);
            var spec = false;
            var options = null;
    ...
    Odoo.prototype.create = function (model, data, options, cb) {
        ...
        this.connect(function (err, response, sid) {
            ...
            if (response.result.uid) {
                ...
                var error = new Error(g.f("Can not get the uid, please check the connect string."))
                cb (error,null);
            }
            ...
            function createResHandle(err, res) {
                if (err) cb(err,null);
                if (res.error) cb(res.error.data,null);
                cb(null, res.result);
            }
        });
    ...
    Odoo.prototype.all = function (model, filter, options, cb) {
        ...
        this.connect(function (err, response, sid) {
            ...
            function allResHandle(err, res) {
                if (err) cb(err,null);
                if (res.error || !res.result) cb(res.error.data,null);
                cb(null, res.result.records);
            }
        });
    };
    ...
    Odoo.prototype.save = function (model, data, options, cb) {
        ...
        this.connect(function (err, response, sid) {
            ...
            var saveArgs = [];
            //todo:what if the id is not exited?
            saveArgs.push([data.id,data]);

            self._write(model,saveResHandle,'write',saveArgs);
